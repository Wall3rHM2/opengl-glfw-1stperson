...
float CamController::pitch2Ang(float pitch) //convert pitch values from rad to local player angle
{
	return  pitch * (90.f / ((float)M_PI / 2.f)); // coverts to local angles and store

}
float CamController::pitch2Rad(float pitch) //convert pitch values from local player angle to rad
{
	return  pitch / (90.f / ((float)M_PI / 2.f)); //

}

float CamController::yaw2Ang(float yaw)
{
	return  yaw * (90.f / ((float)M_PI / 2.f));

}
float CamController::yaw2Rad(float yaw) //convert pitch values from rad to local player angle
{
	return  yaw / (90.f / ((float)M_PI / 2.f)); // coverts to local angles and store

}

glm::vec3 CamController::FromSourcePos(glm::vec3 pos) //convert from source engine coordinates
{
...
glm::mat4 CamController::LookAt()
{
	...
	Direction = glm::rotate(Direction, yaw2Rad(Yaw), glm::vec3(0, 1, 0));
...
{
	if (Yaw + TURN_RATE > 180.f) Yaw = -180.f;
	Yaw += TURN_RATE;
